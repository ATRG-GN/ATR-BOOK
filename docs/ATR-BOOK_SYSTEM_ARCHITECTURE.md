...
#### แนวทางการประสานงาน AI เพื่อ Smooth UX
- **Streaming Responses**
  - ระดับ repository ควร expose เป็น `Stream<String>` เพื่อส่งผลลัพธ์แบบ chunk-by-chunk
  - UI แสดงผลข้อความได้ทันทีโดยไม่ต้องรอทั้ง response ช่วยลดความรู้สึกหน่วง
- **Token Budgeting + Fallback**
  - เพิ่ม `TokenEstimator` ใน domain/service layer เพื่อประเมิน context ก่อนส่ง API
  - หากเกินโควตา ให้ใช้กลยุทธ์ `Summarize-to-Shorten` เพื่อลด context โดยคงสาระสำคัญ
  - เก็บ telemetry เรื่อง token usage ต่อคำขอ เพื่อควบคุมต้นทุนและ tuning model ได้ต่อเนื่อง
- **TTS Pipeline แบบ Prefetching**
  - ขณะเล่นประโยคปัจจุบัน ให้ prefetch audio ของประโยคถัดไป
  - ลดช่องว่างระหว่างประโยคและทำให้ประสบการณ์การฟังต่อเนื่อง

#### ข้อเสนอ BLoC สำหรับฟีเจอร์ AI Assistant
- **Events**
  - `PromptSubmitted`
...
- **States**
  - `AiIdle`
  - `AiGenerating(progress: double)`
  - `AiSuccess`
  - `AiFailure(error: DomainFailure)`
- แนวทางนี้ช่วยแยก business logic ออกจาก UI อย่างชัดเจน และกำหนด retry/cancel policy ได้จาก layer เดียว
...
- **Web**
  ...
  - ใช้ backend proxy เป็นผู้ถือ key จริง และให้ web app ใช้งานผ่าน short-lived JWT
  - เสริมมาตรการป้องกัน XSS/CSRF ตามมาตรฐานเว็บก่อนเปิดใช้งาน production
